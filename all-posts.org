...
** TODO The ~*Completions*~ Buffer Gets a Big Upgrade in Emacs 29
:properties:
:export_file_name: emacs-29-completions
:export_date: <2022-12-31 Sat>
:end:

I've been an ardent [[https://github.com/minad/vertico][vertico]] + [[https://github.com/oantolin/orderless][orderless]] + [[https://github.com/minad/marginalia/][marginalia]] + [[https://github.com/minad/corfu][corfu]] user since
seriously picking up Emacs over the summer.  When initially looking for options
I found [[https://protesilaos.com/emacs/mct][Prot's MCT]] pretty alluring, but didn't choose it since he had already
decided to [[https://protesilaos.com/codelog/2022-04-14-emacs-discontinue-mct/][discontine development]] given upcoming changes in Emacs 29.  As of
writing, even he opted for [[https://git.sr.ht/~protesilaos/dotfiles/tree/437a303b90b3354ca1a1d08cb2f793183d1b4c48/item/emacs/.emacs.d/prot-emacs-modules/prot-emacs-completion.el#L141][vertico]] and [[https://git.sr.ht/~protesilaos/dotfiles/tree/437a303b90b3354ca1a1d08cb2f793183d1b4c48/item/emacs/.emacs.d/prot-emacs-modules/prot-emacs-completion.el#L300][corfu]].
...
another try, so that you don't have to.  Some verbiage below is taken directly from ~C-h n~ (~view-emacs-news~)
...
above, also notice that the buffer updates with contents of the ~python~ directory
...
file:content/auto-help-always.gif
...
\ast{}Completions\ast{} buffer on the first TAB press:

#+begin_src emacs-lisp :results none
(setq completion-auto-help 'visible)
#+end_src

file:content/auto-help-visible.gif
 ...
 * ~C-x <up>~ (~minibuffer-complete-history~) is like ~minibuffer-complete~ but
   ...
   ~C-x <down>~ (~minibuffer-complete-defaults~) completes on the list of
   default items.

All of the above also work for for ~completion-at-point~ (in-buffer completion).

the arrow keys are an unfortunate choice.  we might want to use something more
convenient:
...
file:content/auto-switch.gif

*** The "\ast{}Completions\ast{}" buffer can now be automatically selected.
Set ~completion-auto-select~ to one of ~t~ or ~second-tab~ to enable automatic
selection of the "\ast{}Completions\ast{}" buffer

#+begin_src emacs-lisp :results none
(setq completion-auto-select t)
#+end_src

file:content/auto-select-t.gif
...
file:content/auto-select-second-tab.gif

With 'second-tab', we can use the "\ast{}Completions\ast{}" buffer a lot like how we might
use ~corfu~.  We can type a bit, request completion with TAB, examine the list,
and keep typing to narrow the candidates, without needing to manually exit the
"\ast{}Completions\ast{}" buffer.
...
Some more nicities added this time around:
 ...
 * Using ~C-u RET~ on a completion candidate in the "\ast{}Completions\ast{}" buffer inserts
   the completion to the minibuffer, but doesn't exit the minibuffer.
...
So will I stick with just \ast{}Completions\ast{}?  No, probably not.  But I will give it
an honest chance to see just how far I can push it, [[https://github.com/protesilaos/mct#12-alternatives][much in the spirit of MCT]],
